exports.handler = async function (context, event, callback) {
  ...
  async function callNextAvailableAgent(AgentAttempt, originCallSid) {
    const attempt = Number(AgentAttempt || 0) + 1;
    ...
    const sayStr = `You have a call from ${userNum
      .split("")
      .join(". ")}. Press 1 to accept.`;
    const getParams = `agentNum=${encodeURIComponent(
      agentNum
    )}&amp;userNum=${encodeURIComponent(
      userNum
    )}&amp;callerId=${encodeURIComponent(
      callerId
    )}&amp;originCallSid=${oCallSid}`;
    ...
    const agentCall = await client.calls.create({
      ...
      to: agentNum,
      from: callerId,
      statusCallback: `${baseUrl}?AgentResponse=noresponse&AgentAttempt=${attempt}&userNum=${encodeURIComponent(
        userNum
      )}&callerId=${encodeURIComponent(callerId)}&originCallSid=${oCallSid}`,
  ...
  async function handleInitialInboundCall() {
    for (const i in getListOfAgentsToTry()) {
      await callNextAvailableAgent(i, event.CallSid);
    }
    // puts the current caller on hold, the above agentCall will connect after agents picks up and hears the message.
    const response = new Twilio.twiml.VoiceResponse();
    // response.say(
    //   "Thank you for calling Zimmer Conference line. Please wait while we connect other participants."
    // );
  ...
  async function handleDigitsFromCall() {
    if (event.Digits == 1) {
      const agentCall = await client.calls(event.CallSid).update({
        url: `${baseUrl}?AddAgentToQueue=true&userNum=${encodeURIComponent(
          event.userNum
        )}`,
